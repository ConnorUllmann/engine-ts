...
export class QuadTree {
  ...
  remove(id: number) {
    if (!(id in this.rectangleById)) return;
  ...
  getLeafRectangles(): DeepReadonly<IRectangle[]> {
    ...
    const gatherRectangles = (node: QuadNode) => {
      if (node.hasChildren) node.children.forEach(o => gatherRectangles(o));
      else rectangles.push(node);
    };
...
class QuadNode implements IRectangle, IPoolable {
  ...
  constructor(
    ...
    public w: number,
    public h: number,
    public splitUsingMidpoint = true
  ) {
    this.tree = tree;
  ...
  private passEntitiesToChildren(): void {
    for (let id of this.ids) {
      const rectangle = this.tree.rectangleById[id];
      for (let child of this.children) child.insert(id, rectangle);
    }
    this.ids.clear();
  }
  ...
  private getChildIds(): Set<number> | null {
    ...
    for (let child of this.children) {
      if (child.hasChildren) return null;
      for (let id of child.ids) this.childIdsTemp.add(id);
    }
  ...
  remove(id: number): boolean {
    ...
    if (this.hasChildren) {
      if (
        ...
        for (let child of this.children) if (child.remove(id)) shouldTryToMerge = true;

        if (shouldTryToMerge) {
          const childIds = this.getChildIds();
          if (childIds != null && childIds.size <= this.tree.maxEntitiesPerNode) {
            for (let id of this.childIdsTemp) this.ids.add(id);
            this.children.clear();
            return true;
          }
        }
  ...
  insert(id: number, rectangle: IRectangle): void {
    ...
    if (this.hasChildren) this.children.forEach(o => o.insert(id, rectangle));
    else {
      this.tree.rectangleById[id] = rectangle;
      this.ids.add(id);
      const canSplit =
        this.ids.size > this.tree.maxEntitiesPerNode &&
        this.w / 2 >= this.tree.minNodeSideLength &&
        this.h / 2 >= this.tree.minNodeSideLength;
      if (canSplit) {
        if (this.splitUsingMidpoint) this.splitMidpoint();
        else this.splitCenter();
      }
    }
  }

  queryPoint(x: number, y: number, ids: Set<number>): void {
    if (!Geometry.CollideExplicit.RectanglePoint(this.x, this.y, this.w, this.h, x, y)) return;

    if (this.hasChildren) this.children.forEach(o => o.queryPoint(x, y, ids));
    else if (this.ids != null) this.ids.forEach(id => ids.add(id));
  }

  queryRectangle(x: number, y: number, w: number, h: number, ids: Set<number>): void {
    if (!Geometry.CollideExplicit.RectangleRectangle(this.x, this.y, this.w, this.h, x, y, w, h)) return;

    if (this.hasChildren) this.children.forEach(o => o.queryRectangle(x, y, w, h, ids));
    else if (this.ids != null) this.ids.forEach(id => ids.add(id));
  }
}
