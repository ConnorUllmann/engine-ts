...
export class MarkovChain {
  ...
  public update() {
    ...
    if (this.current != null) this.current.execute();
  }
}
...
export class MarkovLink {
  ...
  public sample(_rng?: RNG): MarkovLink | null {
    if (this.linkWeights.length <= 0) return null;
    ...
    for (const { link, weight } of this.linkWeights) {
      if (partialSum + weight >= selection) return link;
      partialSum += weight;
    }
    throw new Error(
      `Failed to sample for the next markov link. partialSum: ${partialSum}, selection: ${selection}, max: ${max}, linkWeights: ${JSON.stringify(
        this.linkWeights
      )}`
    );
  }
