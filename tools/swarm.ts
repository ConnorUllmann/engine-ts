...
export class Swarm {
  ...
  public addPredator(predator: Entity) {
    this.predators.push(predator);
  }
  public removePredator(predator: Entity) {
    this.predators.remove(predator);
  }
  public addSwarmer(swarmer: Entity) {
  ...
  public removeSwarmer(swarmer: Entity) {
    ...
    if (swarmerFound) this.swarmInstincts.remove(swarmerFound);
  }

  public update() {
    for (const swarmInstinct of this.swarmInstincts) swarmInstinct.updateAngle(this.swarmInstincts, this.predators);
  }
  ...
  public isAvoidingPredator(swarmer: Entity): boolean {
    return this.swarmInstincts.length <= 0
      ? false
      : this.swarmInstincts.first(o => o.swarmer === swarmer)!.isAvoidingPredator;
  }
...
class SwarmInstinct {
  ...
  public updateAngle(swarmInstincts: SwarmInstinct[], predators: Entity[]) {
    ...
    const visiblePredators = predators.filter(
      o => Geometry.Point.DistanceSq(o.position, this.swarmer.position) <= predatorAvoidanceRadiusSq
    );
    const predatorDirection: Point = Point.Create(
      Geometry.Points.Sum(
        visiblePredators.map(o =>
          Geometry.Point.Normalized(
            Geometry.Point.Subtract(this.swarmer.position, o.position),
            this.repulsionMultiplier
          )
        )
      )
    );
    if (predatorDirection.lengthSq > 0.01) {
      ...
      this.angle = Geometry.Point.Angle(
        Geometry.Points.Sum(swarmInstincts.map(swarmInstinct => this.swarmVectorForNeighbor(swarmInstinct)))
      );
    }
  }

  public swarmVectorForNeighbor(neighborSwarmInstinct: SwarmInstinct): IPoint {
    if (this.swarmer === neighborSwarmInstinct.swarmer) return { x: 0, y: 0 };
    ...
    if (isRepulsed) this.isRepulsed = true;
