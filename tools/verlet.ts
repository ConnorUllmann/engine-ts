...
export class VerletStick implements IVerletStick<IVerletPoint> {
  ...
  constructor(
    public a: IVerletPoint,
    public b: IVerletPoint,
    length: number | null = null,
    public stiffness: number = 2
  ) {
...
export class VerletSystem<P extends IVerletPoint, S extends IVerletStick<P>> {
  ...
  updatePoints(): this {
    for (const verletPoint of this.verletPointSet) verletPoint.update();
    return this;
  }

  updateSticks(): this {
    for (const verletStick of this.verletStickSet) verletStick.update();
    return this;
  }
