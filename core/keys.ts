...
export function KeysForKeyCode(keyCode: number, code?: string): Key[] {
  ...
  switch (keyCode) {
    case 13:
      if (code === 'Enter') result.push(Key.ENTER, Key.RETURN);
      if (code === 'NumpadEnter') result.push(Key.NUMPAD_ENTER);
      break;
    case 16:
      result.push(Key.SHIFT);
      if (code === 'ShiftLeft') result.push(Key.LSHIFT);
      else if (code === 'ShiftRight') result.push(Key.RSHIFT);
      break;
    case 17:
      result.push(Key.CTRL);
      if (code === 'ControlLeft') result.push(Key.LCTRL);
      else if (code === 'ControlRight') result.push(Key.RCTRL);
      break;
    case 18:
      result.push(Key.ALT);
      if (code === 'AltLeft') result.push(Key.LALT);
      else if (code === 'AltRight') result.push(Key.RALT);
      break;
    case 45:
      if (code === 'Numpad0') result.push(Key.NUMPAD_ZERO);
      else if (code === 'Insert') result.push(Key.INSERT);
      break;
    case 35:
      if (code === 'Numpad1') result.push(Key.NUMPAD_ONE);
      else if (code === 'End') result.push(Key.END);
      break;
    case 40:
      if (code === 'Numpad2') result.push(Key.NUMPAD_TWO);
      else if (code === 'ArrowDown') result.push(Key.DOWN);
      break;
    case 34:
      if (code === 'Numpad3') result.push(Key.NUMPAD_THREE);
      else if (code === 'PageDown') result.push(Key.DOWN);
      break;
    case 37:
      if (code === 'Numpad4') result.push(Key.NUMPAD_FOUR);
      else if (code === 'ArrowLeft') result.push(Key.LEFT);
      break;
    case 12:
      if (code === 'Numpad5') result.push(Key.NUMPAD_FIVE);
      break;
    case 39:
      if (code === 'Numpad6') result.push(Key.NUMPAD_SIX);
      else if (code === 'ArrowRight') result.push(Key.RIGHT);
      break;
    case 36:
      if (code === 'Numpad7') result.push(Key.NUMPAD_SEVEN);
      else if (code === 'Home') result.push(Key.HOME);
      break;
    case 38:
      if (code === 'Numpad8') result.push(Key.NUMPAD_EIGHT);
      else if (code === 'ArrowUp') result.push(Key.UP);
      break;
    case 33:
      if (code === 'Numpad9') result.push(Key.NUMPAD_NINE);
      else if (code === 'PageUp') result.push(Key.PAGE_UP);
      break;
    case 46:
      if (code === 'NumpadDecimal') result.push(Key.NUMPAD_DECIMAL);
      else if (code === 'Delete') result.push(Key.DELETE);
      break;
    default:
      if (keyCode in KeysByKeyCode) result.push(...KeysByKeyCode[keyCode]);
      break;
  }
  return result;
}

export const CharactersByKeyCode: {
  [key: number]: { lower: string; upper: string };
} = {
  [9]: { lower: '\t', upper: '\t' },
  [13]: { lower: '\n', upper: '\n' },
  [32]: { lower: ' ', upper: ' ' },
