...
import { frictionWithDelta } from '../tools/percent-with-delta';
...
export abstract class Actor extends Entity {
  ...
  public get boundsLocal(): Readonly<IRectangle> {
    return this.collider.boundsLocal;
  }
  ...
  public get xLeft(): number {
    return this.collider.xLeft;
  }
  public set xLeft(x: number) {
    this.position.x = x - this.boundsLocal.x;
  }
  public get xCenter(): number {
    return this.collider.xCenter;
  }
  public set xCenter(x: number) {
    this.position.x = x - this.boundsLocal.w / 2 - this.boundsLocal.x;
  }
  public get xRight(): number {
    return this.collider.xRight;
  }
  public set xRight(x: number) {
    this.position.x = x - this.boundsLocal.w - this.boundsLocal.x;
  }
  public get yTop(): number {
    return this.collider.yTop;
  }
  public set yTop(y: number) {
    this.position.y = y - this.boundsLocal.y;
  }
  public get yCenter(): number {
    return this.collider.yCenter;
  }
  public set yCenter(y: number) {
    this.position.y = y - this.boundsLocal.h / 2 - this.boundsLocal.y;
  }
  public get yBottom(): number {
    return this.collider.yBottom;
  }
  public set yBottom(y: number) {
    this.position.y = y - this.boundsLocal.h - this.boundsLocal.y;
  }
  ...
  protected updateVelocity() {
    this.velocity.add(this.frameAcceleration).scale(frictionWithDelta(1 - this.friction, this.world.deltaNormal));
  }
  ...
  protected updatePosition() {
    this.position.add(this.frameVelocity);
  }
  ...
  get frameVelocity(): IPoint {
    ...
    return this._frameVelocity.setToXY(this.velocity.x * deltaNormal, this.velocity.y * deltaNormal);
  }

  private _frameAcceleration: Point = new Point();
  get frameAcceleration(): IPoint {
    const deltaNormal = this.world.deltaNormal;
    return this._frameAcceleration.setToXY(this.acceleration.x * deltaNormal, this.acceleration.y * deltaNormal);
  }
