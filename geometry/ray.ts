...
export class Ray implements IRay {
  ...
  public get hash(): string {
    return Geometry.Ray.Hash(this);
  }
  public isEqualTo(ray: IRay): boolean {
    return Geometry.Ray.AreEqual(this, ray);
  }
  public get slope(): number {
    return Geometry.Ray.Slope(this);
  }
  public asSegment(length: number = Ray.DefaultMaxDistance): Segment {
    return Segment.Create(Geometry.Ray.AsSegment(this, length));
  }
  public pointAtDistance(length: number = Ray.DefaultMaxDistance): Point {
    return Point.Create(Geometry.Ray.PointAtDistance(this, length));
  }
  public YatX(x: number): number | null {
    return Geometry.Ray.YatX(this, x);
  }
  public XatY(y: number): number | null {
  ...
  public cast<T extends ISegment>(
    segments: T[],
    maxDistance: number = Ray.DefaultMaxDistance
  ): IRaycastResult<ISegment> | null {
    return Geometry.Ray.Cast(this, segments, maxDistance);
  }

  constructor(public a: Point, public angle: number) {}
  public cloneRay(): Ray {
    return new Ray(this.a.clone, this.angle);
  }
}
